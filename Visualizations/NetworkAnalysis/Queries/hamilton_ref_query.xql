...
let $songs := $hamilton//text[descendant::sp/@who[tokenize(string(), ' ') = $i]]
let $song := $songs//head/title

for $s in $song
let $selectSong := $songs[descendant::head/title = $s]
...
let $filename := "hamilton_ref_data.tsv"
